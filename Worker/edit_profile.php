...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "UPDATE mp_online_service_worker SET 
            username = ?, email = ?, phone_number = ?, address = ?, 
            service_type = ?, experience = ?, availability = ? 
            WHERE id = ?";
    
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssii", $username, $email, $phone, $address, $service_type, $experience, $availability, $user_id);
    ...
    if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] == 0) {
        ...
        if (in_array(strtolower($filetype), $allowed)) {
            ...
            if (move_uploaded_file($_FILES['profile_picture']['tmp_name'], $upload_path)) {
                $sql = "UPDATE mp_online_service_worker SET profile_picture = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
    ...
    <div class="edit-profile-container">
        ...
        <form action="" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" id="service_type" name="service_type" value="<?php echo htmlspecialchars($user['service_type']); ?>" required>
            </div>
            <div class="form-group">
                <label for="experience">Experience (years):</label>
                <input type="number" id="experience" name="experience" value="<?php echo htmlspecialchars($user['experience']); ?>" required>
            </div>
            <div class="form-group">
                <label for="availability">Availability:</label>
                <input type="text" id="availability" name="availability" value="<?php echo htmlspecialchars($user['availability']); ?>" required>
            </div>
